...
def bestStudentAndAvg(gradebook):
    ...
    for line in gradebook.splitlines():
        ...
        if not line.startswith("#"):
            ...
            if index != 0 and bestAvg == None or currentSum/index >= bestAvg:
                bestAvg = currentSum/index
                bestStu = currentStu
...
def playPig():
    ...
    while score1 < 100 and score2 < 100:
        ...
        while player == currentplayer:
            ...
            #if player rolls 1 change player without adding 
            #anything to the player's total score
            if number==1: currentplayer=2 if currentplayer==1 else 1
            else:
                currentsum+=number
                action = input("Do you want to hold or continue? >>> ")
                #if player choose hold, add current sum of score 
                #to player's total score.
                if action == "hold":
                    if currentplayer==1: score1+=currentsum 
                    elif currentplayer==2: score2+=currentsum   
                    currentplayer=2 if currentplayer==1 else 1
        print("P1's score = "+str(score1)+"\n"+ "P2's score = "+str(score2))
...
def testCollapseWhiltespace():
    print("---Testing collapseWhitespace")
